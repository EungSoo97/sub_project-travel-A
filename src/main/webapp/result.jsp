...
<div class="container">
  ...
  <c:if test="${not empty result and result.success}">
    ...
    <div class="card summary-header">
      <h1 style="margin:0;">✈️ ${result.summary.title}</h1>
      <p style="font-size: 18px; opacity: 0.9;">${result.summary.overview}</p>
      <div style="margin-top: 15px;">
        <span style="font-weight: 600;">📍 ${result.summary.destination}</span> |
        <span>📅 ${result.summary.startDate} ~ ${result.summary.endDate} (${result.summary.days}일)</span> |
        <span>👤 ${result.summary.travelers}인</span>
      </div>
    ...
    <!-- 2. 추천 항공권 (Flights) -->
    <h2 style="margin-bottom:15px;">🎫 추천 항공권</h2>
    <div class="grid">
    ...
    <c:forEach var="day" items="${result.itinerary}">
      ...
      <div style="margin-bottom: 10px; color: #666;">
        🚌 주 교통수단: <b>${day.transportation}</b> | 📏 총 이동: <b>${day.totalDistanceKm}km</b> | 💰 일일 예상: <b><fmt:formatNumber value="${day.estimatedCost}" type="number"/> ${day.currency}</b>
      </div>
      <p style="font-size: 15px; background: #f1f3f5; padding: 10px 15px; border-radius: 8px;">💡 ${day.summary}</p>
          ...
          <div class="activity-item">
            ...
            <div class="card" style="padding: 16px; margin: 10px 0;">
              <div style="font-weight: 800; font-size: 18px; margin-bottom: 5px;">
                [${act.type}] ${act.name}
                <c:if test="${not empty act.rating}"><span style="color:#ffc107; font-size: 14px; margin-left:10px;">★ ${act.rating}</span></c:if>
              </div>
              <div style="color: #666; font-size: 14px; margin-bottom: 10px;">${act.description}</div>
              <div style="font-size: 13px; color: #888;">📍 ${act.location} / ${act.address}</div>
    ...
    <!-- 4. 추천 숙소 (Hotels) -->
    <h2 style="margin-top:40px;">🏨 추천 숙소</h2>
    <div class="grid">
      <c:forEach var="h" items="${result.hotels}">
        <div class="card">
          <h3>${h.name} <span style="color:#ffc107; font-size: 15px;">★ ${h.rating}</span></h3>
