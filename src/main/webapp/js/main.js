...
let progress = 0;
let currentStep = 0;
const totalEstimatedTimeMs = 45000; // 예상 생성 시간: 45초 (Gemini 2.5 JSON 생성기반)
...
form.addEventListener('submit', function(e) {
    ...
    const stepInterval = setInterval(() => {
        if (currentStep < 3) {
            ...
            prevDiv.classList.remove('bg-blue-50', 'border-2', 'border-blue-500', 'scale-105');
            prevDiv.classList.add('bg-gray-100');
            // 아이콘 체크 표시로 변경 (Lucide 아이콘 객체화)
            prevDiv.querySelector('.flex-shrink-0').classList.replace('bg-blue-100', 'bg-blue-600');
            prevDiv.querySelector('.flex-shrink-0').classList.replace('text-blue-600', 'text-white');
            prevDiv.querySelector('.flex-shrink-0').innerHTML = '<i data-lucide="check" class="w-6 h-6"></i>';
            prevDiv.querySelector('h3').classList.replace('text-blue-700', 'text-gray-800');
            ...
            currDiv.classList.add('bg-blue-50', 'border-2', 'border-blue-500', 'scale-105');
            ...
            currDiv.querySelector('h3').classList.replace('text-gray-700', 'text-blue-700');
